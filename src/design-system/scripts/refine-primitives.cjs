...
function processCSSFile(inputFile, outputFile) {
  fs.readFile(inputFile, "utf8", (err, data) => {
    ...
    lines.forEach((line) => {
      ...
      if (variableName && variableValue) {
        ...
        if (isColorCode(variableValue)) {
          ...
          colorsNamesMap[line.split(":")[0].replace(/'/g, "")] = variableName;
        } else {
    ...
    const outputContent = `
    module.exports = {
        colorNamesMap: ${JSON.stringify(colorsNamesMap, null, 2)},
        colors: ${JSON.stringify(colorVariables, null, 2)},
        spacing: ${JSON.stringify(spacingVariables, null, 2)}
    };
`;
...
const inputFile = "../seeds/primitives/index.css";
const outputFile = "../generated/refined-primitives.cjs";
